...
def load_csv(path, cols):
    if os.path.exists(path):
        # keep it simple to avoid encoding/arg issues
...
def bootstrap_files():
    ...
    for fpath, cols in [
        ...
        if not os.path.exists(fpath):
            save_csv(new_df(cols), fpath)
        else:
            df = load_csv(fpath, cols)  # migrate missing columns if any
            save_csv(df, fpath)
...
def app_ui():
    ...
    # non-admin users can access all primary tabs; Users tab only for admin
    ...
    with tab_objs[1]:
        ...
        with col_s1:
            if st.button("💾 Save Inventory Changes", type="primary"):
                ...
                # Generate item_id for new rows missing it
                missing_id_mask = df_save["item_id"].isna() | (df_save["item_id"].astype(str).str.strip()=="")
                for i, r in df_save[missing_id_mask].iterrows():
                    gen_id = hashlib.md5(str(r.get("item_name","")).encode()).hexdigest()[:8]
                    df_save.at[i, "item_id"] = gen_id
                # Coerce numeric + round sell_price
    ...
    with tab_objs[3]:
        ...
        if inv.empty:
            ...
            # Dropdown 1: Category (filters items)
            ...
            inv_filtered = inv if sel_cat == "-- All --" else inv[inv["category"].astype(str) == sel_cat]
            ...
            with st.form("sales_form_menu", clear_on_submit=True):
                ...
                s_customer = st.text_input("Customer mobile", disabled=(s_payment == "Cash"), help="Required only for Credit")
                ...
                # Default price from sell_price; 5-rupee increments
                default_price = 0
            ...
            if s_sub:
                ...
                if s_item_name == "-- Select --":
                ...
                elif s_payment == "Credit" and not s_customer:
                    st.error("Customer mobile is required for Credit.")
                else:
                    customer_id = s_customer if s_payment == "Credit" and s_customer else "GUEST"
                    ...
                    if rate <= 0:
                        ...
                        if not ok_stock:
                            ...
                            # undo dry-run and record properly to keep a single source of stock mutation
    ...
    if user['role']=="admin":
        with tab_objs[-1]:
            ...
            # Show editable grid WITHOUT password hashes
            ...
            colu1, colu2 = st.columns([0.40, 0.60])
            with colu1:
                if st.button("💾 Save User Table", type="primary"):
                    # apply deletes & updates, preserve password_hash for existing user_ids
                    keep = edited_users[~edited_users["DELETE"]].drop(columns=["DELETE"]).copy()

                    # Start from original users; update rows by user_id; add new ones (with temp password)
                    original = users_full.copy()

                    # Update existing
                    for _, r in keep.iterrows():
                        uid = str(r["user_id"]).strip() if pd.notna(r["user_id"]) else ""
                        if uid and (original["user_id"] == uid).any():
                            ...
                            # new user row -> set default password "welcome123"
                            if not uid:
                                continue
                            new_row = {
                                "user_id": uid,
                    ...
                    # Delete rows not present in keep (by user_id)
                    ...
                    st.success("Users saved. (New users get default password: welcome123)")
                    st.rerun()

            with colu2:
                st.markdown("**Add / Reset Password**")
