...
# Allow all roles to access all tabs (set False to restrict to admin)
ALLOW_ALL_TABS_FOR_NON_ADMIN = True

# Inventory has sell_price (5-rupee step)
SCHEMA = {
...
def load_csv(path, cols):
    """
    Robust CSV loader:
    - If file exists, try utf-8; fallback to latin1.
    - Ensure required columns exist (add missing).
    - Return exactly the requested columns in order.
    """
    if os.path.exists(path):
        ...
        for c in cols:
            if c not in df.columns:
                df[c] = None
        # return only known columns (drop unknowns)
        return df[cols]
    return new_df(cols)
...
def round_to_5(n) -> int:
    """Round any number to nearest ₹5 integer."""
    try:
        x = float(n)
...
def app_ui():
    ...
    # Tabs (admins + optionally all roles)
    ...
    with tab_objs[1]:
        ...
        with st.expander("📤 Import CSV (Inventory or Expenses)", expanded=False):
            mode = st.radio("Choose import target", ["Inventory", "Expenses"], horizontal=True)
            if mode == "Inventory":
                st.info("CSV must have columns: Item Name, Category, Unit, Suppliers Rate")
                sample_data = pd.DataFrame({
                    'Item Name': ['Advance Compact','Airtight Containers'],
                    'Category': ['Menu','Menu'],
                    'Unit': ['pack','set'],
                    'Suppliers Rate': [75.0, 50.0]
                })
            else:
                st.info("CSV must have columns: Date (YYYY-MM-DD), Category, Expense Item, Amount, Remarks")
                sample_data = pd.DataFrame({
                    'Date': [datetime.now().date().isoformat()],
                    'Category': ['Utilities'],
                    'Expense Item': ['Electricity'],
                    'Amount': [1200],
                    'Remarks': ['August bill']
                })
            st.download_button("📥 Download Sample CSV",
                               data=sample_data.to_csv(index=False).encode('utf-8'),
                               file_name=f"{mode.lower()}_sample.csv",
                               mime='text/csv')
                    ...
                    if st.button("Process Import", key="import_btn", type="primary"):
                        if mode == "Inventory":
                            ...
                            for _, row in df_import.iterrows():
                                ...
                                rate = pd.to_numeric(row.get('Suppliers Rate',0), errors='coerce')
                                rate = 0 if pd.isna(rate) else float(rate)
                                if not name:
                                    continue
                                ...
                                if existing.empty:
                                    new_row = {
                                        "item_id": item_id, "item_name": name, "category": category, "unit": unit,
                            ...
                            st.success(f"✅ Inventory import done. Processed:{processed} Added:{added} Updated:{updated}")
                            st.rerun()
                        else:
                            # Expenses import
                            required = ['Date','Category','Expense Item','Amount']
                            for col in required:
                                if col not in df_import.columns:
                                    st.error(f"Missing column: {col}")
                                    st.stop()
                            df_exp = load_csv(EXPENSES_FILE, SCHEMA["expenses"])
                            for _, r in df_import.iterrows():
                                dt = str(r['Date'])
                                cat = str(r['Category'])
                                item = str(r['Expense Item'])
                                amt = pd.to_numeric(r['Amount'], errors='coerce')
                                amt = 0 if pd.isna(amt) else float(amt)
                                remarks = str(r.get('Remarks',""))
                                df_exp.loc[len(df_exp)] = [dt, "Expense", cat, item, "", 0.0, 0.0, amt, user.get("user_id",""), remarks]
                            save_csv(df_exp, EXPENSES_FILE)
                            st.success("✅ Expenses import done.")
                            st.rerun()
        ...
        edited = st.data_editor(
            ...
            num_rows="dynamic",
            column_config={
                "DELETE": st.column_config.CheckboxColumn(help="Tick to delete row"),
                "item_id": st.column_config.TextColumn(help="Leave blank to auto-generate from name"),
                "item_name": st.column_config.TextColumn(help="Name shown on Menu"),
                "category": st.column_config.TextColumn(help="Use 'Menu' to appear in Menu/Booking"),
                "unit": st.column_config.TextColumn(help="kg/pack/pcs"),
                ...
                "sell_price": st.column_config.NumberColumn(help="Selling price (₹, 5-step)", format="%d", step=5),
            },
            key="inv_editor"
        )

        c_save, c_right = st.columns([0.25, 0.75])
        with c_save:
    ...
    with tab_objs[3]:
        ...
        inv = list_inventory()
        # Only items with category == "Menu" are shown
        inv_menu = inv[inv["category"].astype(str).str.strip().str.lower() == "menu"].copy()
        if inv_menu.empty:
            st.info("No items in category 'Menu'. Go to Inventory and set Category='Menu' for items you want to sell here.")
        else:
            # Item dropdown: show names only
            name_to_id = {r["item_name"]: r["item_id"] for _, r in inv_menu.iterrows()}
            ...
            with st.form("sales_form_menu", clear_on_submit=True):
                ...
                # Customer mobile required only for Credit
                s_customer = st.text_input("Customer mobile (required for Credit only)")
                s_item_name = st.selectbox("Item", options=item_names)
                s_qty = st.number_input("Qty", min_value=1, step=1, value=1, format="%d")
                # default price from sell_price (5 step)
                default_price = 0
                if s_item_name != "-- Select --":
                    default_price = int(inv_menu[inv_menu["item_name"] == s_item_name]["sell_price"].iloc[0])
            ...
            if s_sub:
                ...
                elif s_payment == "Credit" and not s_customer.strip():
                    st.error("Customer mobile is required for Credit")
                else:
                    customer_id = s_customer.strip() if s_payment == "Credit" else "GUEST"
                    pid = name_to_id[s_item_name]
                    item = inv_menu[inv_menu["item_id"].astype(str) == str(pid)].iloc[0]
                    rate = int(item["sell_price"]) if use_item_price else int(round_to_5(s_rate))
                    if rate <= 0:
                        st.error("Price must be positive")
                    else:
                        # pre-check stock before recording (without changing file if fail)
                        have = float(inv[inv["item_id"]==pid]["stock_qty"].iloc[0])
                        if have < float(s_qty):
                            insufficient_stock_dialog(item["item_name"], have, float(s_qty))
                        else:
                            ok, msg = record_order(
                                s_date, customer_id, pid, item["item_name"], s_qty, rate, s_payment,
    ...
    # ----- Users (Admin or allowed) -----
    if user['role']=="admin" or ALLOW_ALL_TABS_FOR_NON_ADMIN:
        with tab_objs[-1]:
            st.header("User Management")
            users = load_csv(USERS_FILE, SCHEMA["users"])
            show = users[["user_id","name","role","mobile"]].copy()
            ...
            edited_users = st.data_editor(
                ...
                column_config={
                    ...
                    "user_id": st.column_config.TextColumn(help="Unique ID"),
                    "name": st.column_config.TextColumn(),
                    "role": st.column_config.SelectboxColumn(options=["admin","staff","customer"]),
                    "mobile": st.column_config.TextColumn(help="Login mobile"),
                },
                key="user_editor"
            )
            colu1, colu2 = st.columns([0.3, 0.7])
            with colu1:
                new_pw = st.text_input("Set password for selected/edited user", type="password", help="If empty, existing hash is kept where possible.")
            with colu2:
                if st.button("💾 Save Users", type="primary"):
                    df_save = edited_users.copy()
                    df_save = df_save[~df_save["DELETE"]].drop(columns=["DELETE"])
                    # merge back password_hash by mobile (if unchanged)
                    existing = load_csv(USERS_FILE, SCHEMA["users"])
                    df_save = df_save.merge(existing[["mobile","password_hash"]], on="mobile", how="left")
                    # If a new password provided, apply to ALL rows (simple admin reset)
                    if new_pw.strip():
                        df_save["password_hash"] = hash_pw(new_pw.strip())
                    # fill missing IDs
                    for i, r in df_save[df_save["user_id"].isna() | (df_save["user_id"].astype(str).str.strip()=="")].iterrows():
                        df_save.at[i, "user_id"] = hashlib.md5(str(r.get("mobile","")).encode()).hexdigest()[:8]
                    df_save = df_save[SCHEMA["users"]]
                    save_csv(df_save, USERS_FILE)
